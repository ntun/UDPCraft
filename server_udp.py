...
class Handler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        if (data != "logout\n"):
            ...
            print(data[:-1])
            #print(count)
            model.enqueue(model.on_data, self, data[:-1])
            sock = self.request[1]
            ...
            print(data[:-1])
            print("%d packets received" % count)
            #if (model.enqueue(model.on_connect, self) != False):
    ...
    def run(self):
        while self.running:
            try:
                ...
                soc = self.request[1]
                soc.sendto(data, self.client_address)
...
class Model(object):
    ...
    def on_connect(self, client):
        global clients_ip
        global client_obj
        global clients_conn_status
        #print(client.client_address[0])
        #print("hello")
        if client.client_address[0] not in clients_ip: # or clients_conn_status[client.client_address[0]] == False:
            client_obj[client.client_address[0]] = client;
            clients_conn_status[client.client_address[0]] = True
            client.client_id = self.next_client_id()
            client.nick = 'guest%d' % client.client_id

            clients_ip.add(client.client_address[0])
            #print(clients_ip)
    ...
    def on_disconnect(self, client):
        # clients_conn_status[client.client_address[0]] = False;
